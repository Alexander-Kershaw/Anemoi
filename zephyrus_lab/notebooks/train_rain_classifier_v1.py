...
from boreas.precip_events import add_heavy_rain_flags



# Load Notus curated data
...
target = "heavy_rain_v1"
df = df.dropna(subset=[target]).copy()
y = df[target].astype(int)


# Features (rain is influenced by synoptic + moisture)
features = [
    "temp", "rhum", "pres", "dpres_1h_hpa",
    "wspd_ms", "u_ms", "v_ms", "wdir",
    "q_gkg", "e_hpa", "dewpoint_depression_c",
    "cldc",
]
...
print(colored("Train period: {t_train.iloc[0]} → {t_train.iloc[-1]}", "yellow"))
...
plt.title("Heavy Rain v1 — ROC Curve (LogReg baseline)")
plt.tight_layout()
plt.savefig(out_dir / "rain_v1_roc.png", dpi=150)
plt.show()

plt.figure()
PrecisionRecallDisplay.from_predictions(y_test, p_test)
plt.title("Heavy Rain v1 — Precision-Recall Curve (LogReg baseline)")
plt.tight_layout()
plt.savefig(out_dir / "rain_v1_pr.png", dpi=150)
plt.show()
...
plt.title("Heavy Rain v1 — Calibration Curve")
plt.legend()
plt.tight_layout()
plt.savefig(out_dir / "rain_v1_calibration.png", dpi=150)
plt.show()
...
joblib.dump(
    ...
    model_dir / "rain_classifier_logreg_v1.joblib"
)
print(colored(f"\nSaved model to: {(model_dir / 'rain_classifier_logreg_v1.joblib').resolve()}", "green"))
